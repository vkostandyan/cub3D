...
/*   Updated: 2025/02/19 17:43:29 by vkostand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	get_v_inter(t_cub3D *data, float angle)
{
	...
	if ((check_circle(angle, 1) && step_y < 0) || (!check_circle(angle, 1)
			&& step_y > 0))
		step_y *= -1;
	...
	data->ray.vert_x = v_x;
	data->ray.vert_y = v_y;
	return (sqrt(pow(v_x - data->player.ply_x, 2) + pow(v_y
				- data->player.ply_y, 2)));
}
...
float	get_h_inter(t_cub3D *data, float angle)
{
	...
	if ((check_circle(angle, 0) && step_x > 0) || (!check_circle(angle, 0)
			&& step_x < 0))
		step_x *= -1;
	...
	return (sqrt(pow(h_x - data->player.ply_x, 2) + pow(h_y
				- data->player.ply_y, 2)));
}
