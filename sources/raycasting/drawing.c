...
/*   Updated: 2025/02/19 17:42:01 by vkostand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mlx_pixel_put_img(t_cub3D *data, int x, int y, int color)
{
	...
	if (x >= SCREENWIDTH || y >= SCREENHEIGHT)
		return ;
	dst = data->img_stract.addr + (y * data->img_stract.line_length + x
			* (data->img_stract.bits_per_pixel / 8));
...
t_img	*get_texture(t_cub3D *data)
{
	...
	if (data->ray.flag == 0)
	{
		if (data->ray.ray_angle >= M_PI / 2 && data->ray.ray_angle <= 3 * (M_PI
				/ 2))
			return (&data->tex->we);
		else
			return (&data->tex->ea);
...
unsigned int	get_color_for_draw(int x, int y, t_img *text_img)
{
	...
	{
		dst = text_img->addr + (y * text_img->line_length + x
				* (text_img->bits_per_pixel / 8));
