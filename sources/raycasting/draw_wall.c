...
/*   Updated: 2025/02/19 17:42:17 by vkostand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	get_dx(t_cub3D *data, t_img *text_img)
{
	...
	if (data->ray.flag == 1)
		dx = (int)fmodf((data->ray.horiz_x * (text_img->w / TILE_SIZE)),
				text_img->w);
	else
		dx = (int)fmodf((data->ray.vert_y * (text_img->w / TILE_SIZE)),
				text_img->w);
	return (dx);
}
...
void	draw_floor_and_ceiling(t_cub3D *data, int ray, double tieri_pix,
		double tveri_pix)
{
	int	tmp;
	...
	while (tmp < SCREENHEIGHT)
		mlx_pixel_put_with_checking(data, ray, tmp++,
			data->parse_data->floor_color);
	tmp = 0;
	while (tmp < tieri_pix)
		mlx_pixel_put_with_checking(data, ray, tmp++,
			data->parse_data->ceiling_color);
}
...
void	instal_the_wall(t_cub3D *data, int ray)
{
	...
	data->ray.distance *= cos(check_angle(data->ray.ray_angle
				- data->player.ply_angle));
	wall_h = (TILE_SIZE / data->ray.distance) * ((SCREENWIDTH / 2)
